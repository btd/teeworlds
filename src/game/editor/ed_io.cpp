...
int CEditorMap::Save(const char *pFileName)
{
	char aBuf[256];
...
{
	Reset();
	return m_Map.Load(pFileName, StorageType);
}

int CEditorMap::Load(const char *pFileName, int StorageType)
{
	CDataFileReader DataFile;
	//DATAFILE *df = datafile_load(filename);
	if(!DataFile.Open(pFileName, StorageType))
		return 0;
...
int CEditor::Append(const char *pFileName, int StorageType)
{
	...
	int Err;
	Err = NewMap.Load(pFileName, StorageType);
	if(!Err)
		return Err;

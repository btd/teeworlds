...
void CLayerTiles::Render()
{
	...
	IEngineGraphics::instance()->TextureSet(m_TexID);
...
void CLayerTiles::BrushSelecting(CUIRect Rect)
{
	...
	IEngineGraphics::instance()->SetColor(1.0f, 1.0f, 1.0f, 0.4f);
	Snap(&Rect);
	...
	IEngineTextRender::instance()->Text(0, Rect.x+3.0f, Rect.y+3.0f, 15.0f*m_pEditor->m_WorldZoom, aBuf, -1);
}
...
void CLayerTiles::ShowInfo()
{
	...
	IEngineGraphics::instance()->TextureSet(IClient::instance()->GetDebugFont());
	...
	for(int y = StartY; y < EndY; y++)
		for(int x = StartX; x < EndX; x++)
		{
			...
			if(m_pTiles[c].m_Index)
			{
				...
				IEngineGraphics::instance()->QuadsText(x*32, y*32, 16.0f, 1,1,1,1, aBuf);

				char aFlags[4] = {	m_pTiles[c].m_Flags&TILEFLAG_VFLIP ? 'V' : ' ',
									m_pTiles[c].m_Flags&TILEFLAG_HFLIP ? 'H' : ' ',
									m_pTiles[c].m_Flags&TILEFLAG_ROTATE? 'R' : ' ',
									0};
				IEngineGraphics::instance()->QuadsText(x*32, y*32+16, 16.0f, 1,1,1,1, aFlags);
			}
			x += m_pTiles[c].m_Skip;
		}
}

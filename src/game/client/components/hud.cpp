...
{
	float Half = 300.0f*IEngineGraphics::instance()->ScreenAspect()/2.0f;
	...
	if(!(m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_SUDDENDEATH))
	{
		...
		{
			Time = m_pClient->m_Snap.m_pGameInfoObj->m_TimeLimit*60 - ((IClient::instance()->GameTick()-m_pClient->m_Snap.m_pGameInfoObj->m_RoundStartTick)/IClient::instance()->GameTickSpeed());

			if(m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags & GAMESTATEFLAG_GAMEOVER)
				Time = 0;
		}
		else
			Time = (IClient::instance()->GameTick()-m_pClient->m_Snap.m_pGameInfoObj->m_RoundStartTick)/IClient::instance()->GameTickSpeed();

		str_format(Buf, sizeof(Buf), "%d:%02d", Time/60, Time%60);
		float FontSize = 10.0f;
		float w = IEngineTextRender::instance()->TextWidth(0, FontSize, Buf, -1);
		// last 60 sec red, last 10 sec blink
		if(m_pClient->m_Snap.m_pGameInfoObj->m_TimeLimit && Time <= 60)
		{
			float Alpha = Time <= 10 && (2*time_get()/time_freq()) % 2 ? 0.5f : 1.0f;
			IEngineTextRender::instance()->TextColor(1.0f, 0.25f, 0.25f, Alpha);
		}
		IEngineTextRender::instance()->Text(0, Half-w/2, 2, FontSize, Buf, -1);
...
void CHud::RenderSuddenDeath()
{
	if(m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_SUDDENDEATH)
	{
		...
		float w = IEngineTextRender::instance()->TextWidth(0, FontSize, pText, -1);
		IEngineTextRender::instance()->Text(0, Half-w/2, 2, FontSize, pText, -1);
	}
}
...
void CHud::RenderScoreHud()
{
	...
	if(!(m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER))
	{
		...
		float Whole = 300*IEngineGraphics::instance()->ScreenAspect();
		...
		if(GameFlags&GAMEFLAG_TEAMS && m_pClient->m_Snap.m_pGameDataObj)
		{
			...
			float aScoreTeamWidth[2] = { IEngineTextRender::instance()->TextWidth(0, 14.0f, aScoreTeam[TEAM_RED], -1), IEngineTextRender::instance()->TextWidth(0, 14.0f, aScoreTeam[TEAM_BLUE], -1) };
			int FlagCarrier[2] = { m_pClient->m_Snap.m_pGameDataObj->m_FlagCarrierRed, m_pClient->m_Snap.m_pGameDataObj->m_FlagCarrierBlue };
			float ScoreWidthMax = max(max(aScoreTeamWidth[TEAM_RED], aScoreTeamWidth[TEAM_BLUE]), IEngineTextRender::instance()->TextWidth(0, 14.0f, "100", -1));
			float Split = 3.0f;
			...
			for(int t = 0; t < 2; t++)
			{
				...
				if(t == 0)
					IEngineGraphics::instance()->SetColor(1.0f, 0.0f, 0.0f, 0.25f);
				else
					IEngineGraphics::instance()->SetColor(0.0f, 0.0f, 1.0f, 0.25f);
				...
				// draw score
				IEngineTextRender::instance()->Text(0, Whole-ScoreWidthMax+(ScoreWidthMax-aScoreTeamWidth[t])/2-Split, 245.0f+t*20, 14.0f, aScoreTeam[t], -1);

				if(GameFlags&GAMEFLAG_FLAGS)
				{
					if(FlagCarrier[t] == FLAG_ATSTAND || (FlagCarrier[t] == FLAG_TAKEN && ((IClient::instance()->GameTick()/10)&1)))
					{
						// draw flag
						IEngineGraphics::instance()->BlendNormal();
					...
					else if(FlagCarrier[t] >= 0)
					{
						...
						float w = IEngineTextRender::instance()->TextWidth(0, 10.0f, pName, -1);
						IEngineTextRender::instance()->Text(0, Whole-ScoreWidthMax-ImageSize-3*Split-w, 247.0f+t*20, 10.0f, pName, -1);
		...
		else
		{
			...
			float aScoreWidth[2] = {IEngineTextRender::instance()->TextWidth(0, 14.0f, aScore[0], -1), IEngineTextRender::instance()->TextWidth(0, 14.0f, aScore[1], -1)};
			float ScoreWidthMax = max(max(aScoreWidth[0], aScoreWidth[1]), IEngineTextRender::instance()->TextWidth(0, 14.0f, "10", -1));
			...
			for(int t = 0; t < 2; t++)
			{
				...
				IEngineGraphics::instance()->QuadsBegin();
				if(t == Local)
					IEngineGraphics::instance()->SetColor(1.0f, 1.0f, 1.0f, 0.25f);
				else
					IEngineGraphics::instance()->SetColor(0.0f, 0.0f, 0.0f, 0.25f);
				...
				// draw score
				IEngineTextRender::instance()->Text(0, Whole-ScoreWidthMax+(ScoreWidthMax-aScoreWidth[t])/2-Split, 245.0f+t*20, 14.0f, aScore[t], -1);
				...
				IEngineTextRender::instance()->Text(0, Whole-ScoreWidthMax-ImageSize-Split-PosSize, 247.0f+t*20, 10.0f, aBuf, -1);
			}
		}
	}
}
...
void CHud::RenderWarmupTimer()
{
	...
	if(m_pClient->m_Snap.m_pGameInfoObj->m_WarmupTimer)
	{
		...
		float w = IEngineTextRender::instance()->TextWidth(0, FontSize, Localize("Warmup"), -1);
		IEngineTextRender::instance()->Text(0, 150*IEngineGraphics::instance()->ScreenAspect()+-w/2, 50, FontSize, Localize("Warmup"), -1);
		...
		w = IEngineTextRender::instance()->TextWidth(0, FontSize, Buf, -1);
		IEngineTextRender::instance()->Text(0, 150*IEngineGraphics::instance()->ScreenAspect()+-w/2, 75, FontSize, Buf, -1);
	}
}
...
void CHud::RenderFps()
{
	...
	{
		// calculate avg. fps
		float FPS = 1.0f / IClient::instance()->FrameTime();
		m_AverageFPS = (m_AverageFPS*(1.0f-(1.0f/m_AverageFPS))) + (FPS*(1.0f/m_AverageFPS));
		char Buf[512];
		str_format(Buf, sizeof(Buf), "%d", (int)m_AverageFPS);
		IEngineTextRender::instance()->Text(0, m_Width-10-IEngineTextRender::instance()->TextWidth(0,12,Buf,-1), 5, 12, Buf, -1);
	}
}

void CHud::RenderConnectionWarning()
{
	if(IClient::instance()->ConnectionProblems())
	{
		const char *pText = Localize("Connection Problems...");
		float w = IEngineTextRender::instance()->TextWidth(0, 24, pText, -1);
		IEngineTextRender::instance()->Text(0, 150*IEngineGraphics::instance()->ScreenAspect()-w/2, 50, 24, pText, -1);
	}
}
...
void CHud::RenderTeambalanceWarning()
{
	...
	if(m_pClient->m_Snap.m_pGameInfoObj->m_GameFlags&GAMEFLAG_TEAMS)
	{
		...
		if (g_Config.m_ClWarningTeambalance && (TeamDiff >= 2 || TeamDiff <= -2))
		{
			...
			if(Flash)
				IEngineTextRender::instance()->TextColor(1,1,0.5f,1);
			else
				IEngineTextRender::instance()->TextColor(0.7f,0.7f,0.2f,1.0f);
			IEngineTextRender::instance()->Text(0x0, 5, 50, 6, pText, -1);
...
{
	if(!m_pClient->m_pVoting->IsVoting() || IClient::instance()->State() == IClient::STATE_DEMOPLAYBACK)
		return;

	IEngineGraphics::instance()->TextureSet(-1);
	IEngineGraphics::instance()->QuadsBegin();
	IEngineGraphics::instance()->SetColor(0,0,0,0.40f);
	...
	float tw = IEngineTextRender::instance()->TextWidth(0x0, 6, aBuf, -1);
	IEngineTextRender::instance()->SetCursor(&Cursor, 5.0f+100.0f-tw, 60.0f, 6.0f, TEXTFLAG_RENDER);
	IEngineTextRender::instance()->TextEx(&Cursor, aBuf, -1);

	IEngineTextRender::instance()->SetCursor(&Cursor, 5.0f, 60.0f, 6.0f, TEXTFLAG_RENDER);
	Cursor.m_LineWidth = 100.0f-tw;
	Cursor.m_MaxLines = 3;
	IEngineTextRender::instance()->TextEx(&Cursor, m_pClient->m_pVoting->VoteDescription(), -1);

	// reason
	str_format(aBuf, sizeof(aBuf), "%s %s", Localize("Reason:"), m_pClient->m_pVoting->VoteReason());
	IEngineTextRender::instance()->SetCursor(&Cursor, 5.0f, 79.0f, 6.0f, TEXTFLAG_RENDER|TEXTFLAG_STOP_AT_END);
...
{
	if(!m_pClient->m_Snap.m_pLocalCharacter || IClient::instance()->State() == IClient::STATE_DEMOPLAYBACK)
		return;
...
void CHud::RenderHealthAndAmmo(const CNetObj_Character *pCharacter)
{
	...
	IEngineGraphics::instance()->QuadsDrawTL(Array, i);
	IEngineGraphics::instance()->QuadsEnd();
	...
	IEngineGraphics::instance()->QuadsDrawTL(Array, h);
...
void CHud::RenderSpectatorHud()
{
	...
	IEngineGraphics::instance()->TextureSet(-1);
	IEngineGraphics::instance()->QuadsBegin();
	IEngineGraphics::instance()->SetColor(0.0f, 0.0f, 0.0f, 0.4f);
	...
	IEngineTextRender::instance()->Text(0, m_Width-174.0f, m_Height-13.0f, 8.0f, aBuf, -1);
}
...
void CHud::OnRender()
{
	...
	m_Width = 300.0f*IEngineGraphics::instance()->ScreenAspect();
	m_Height = 300.0f;
	IEngineGraphics::instance()->MapScreen(0.0f, 0.0f, m_Width, m_Height);

...
bool CDataFileReader::Open(boost::shared_ptr < IStorage> pStorage, const char *pFilename, int StorageType)
{
...
bool CDataFileReader::GetCrcSize(boost::shared_ptr < IStorage> pStorage, const char *pFilename, int StorageType, unsigned *pCrc, unsigned *pSize)
{
	IOHANDLE File = pStorage->OpenFile(pFilename, IOFLAG_READ, StorageType);
	if(!File)
		return false;
...
bool CDataFileWriter::Open(boost::shared_ptr < IStorage> pStorage, const char *pFilename)
{

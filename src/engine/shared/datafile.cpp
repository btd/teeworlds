...
bool CDataFileReader::Open(const char *pFilename, int StorageType)
{
	dbg_msg("datafile", "loading. filename='%s'", pFilename);

	IOHANDLE File = IStorage::instance()->OpenFile(pFilename, IOFLAG_READ, StorageType);
	if(!File)
	{
...
bool CDataFileReader::GetCrcSize(const char *pFilename, int StorageType, unsigned *pCrc, unsigned *pSize)
{
	IOHANDLE File = IStorage::instance()->OpenFile(pFilename, IOFLAG_READ, StorageType);
	if(!File)
		return false;
...
bool CDataFileWriter::Open(const char *pFilename)
{
	dbg_assert(!m_File, "a file already exists");
	m_File = IStorage::instance()->OpenFile(pFilename, IOFLAG_WRITE, IStorage::TYPE_SAVE);
	if(!m_File)
		return false;
